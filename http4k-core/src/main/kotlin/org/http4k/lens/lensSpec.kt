 ...
 */
inline fun <TARGET : Any, reified T> BiDiLensSpec<TARGET, String>.composite(crossinline fn: BiDiLensSpec<TARGET, String>.(TARGET) -> T) = compose(fn).required(T::class.java.name)
...
) = compose(getFn, setFn).required(T::class.java.name)


inline fun <TARGET : Any, reified T> BiDiLensSpec<TARGET, String>.compose(crossinline fn: BiDiLensSpec<TARGET, String>.(TARGET) -> T) = LensSpec<TARGET, T>(
    T::class.java.name, ParamMeta.ObjectParam, LensGet { _, target -> listOf(fn(target)) })

inline fun <TARGET : Any, reified T> BiDiLensSpec<TARGET, String>.compose(
    crossinline getFn: BiDiLensSpec<TARGET, String>.(TARGET) -> T,
    crossinline setFn: T.(TARGET) -> TARGET
) = BiDiLensSpec(
